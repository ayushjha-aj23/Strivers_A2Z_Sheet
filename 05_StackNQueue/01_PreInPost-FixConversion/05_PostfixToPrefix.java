...
class Solution {
    ...
    // Step4: If character is operator ---> Take the last 2 elements of the Stack and insert operator before them as ---> operator + secondLast + last
    ...
    static String postToPre(String post_exp) {
        ...
        for(int i=0; i<post_exp.length(); i++){
            ...
            if( (c>='A' && c<='Z') || (c>='a' && c<='z') || (c>='0' && c<='9') ){
                ...
                String newString = c + secondLast + last;
                st.push(newString);
            }
        }
        return st.pop(); // Return the only element present in the stack
    }
}
